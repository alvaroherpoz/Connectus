...
\n\t\t\t\tsignalIn=${this.getComponentClass(sourceNode, localNodeName)}::${sourceSignal.name.replace(/\s/g, '')}; break;`;
            ...
            return `
TEDROOMSignal CEDROOMSystemCommSAP::C${sourceNode.data.componentId}${sourceName}_P${port_source_name}_C${targetNode.data.componentId}${targetName}_P${port_target_name}(TEDROOMSignal signalOut){
...
TEDROOMSignal CEDROOMSystemCommSAP::C${targetNode.data.componentId}${targetName}_P${port_target_name}_C${sourceNode.data.componentId}${sourceName}_P${port_source_name}(TEDROOMSignal signalOut){
    TEDROOMSignal signalIn;
    switch(signalOut){
    ${reverseCases.trim()}
        default: signalIn=(TEDROOMSignal)(-1); break;
...
`;
    ...
    m_localCommSAP.Connect(mp_${sourceInstance}->${sourcePort}, mp_${targetInstance}->${targetPort}, connections[${index}],
                                 C${sourceNode.data.componentId}${sourceName}_P${port_source_name}_C${targetNode.data.componentId}${targetName}_P${port_target_name},
                                 C${targetNode.data.componentId}${targetName}_P${port_target_name}_C${sourceNode.data.componentId}${sourceName}_P${port_source_name});\n`;
        }).join('\n');
        ...
        const setRemoteConnectionsContent = remoteConnections.map((conn, index) => {
            ...
            return `\
              m_remoteCommSAP.Connect(mp_${localInstance}->${localInterfaceName}, mp_${remoteInstance}->${remoteInterfaceName}, remote_connections[${index}],
                                          C${localComponent.data.componentId}${localName}_P${local_interface_name}_C${remoteComponent.data.componentId}${remoteName}_P${remote_interface_name},
                                          C${remoteComponent.data.componentId}${remoteName}_P${remote_interface_name}_C${localComponent.data.componentId}${localName}_P${local_interface_name});`;
